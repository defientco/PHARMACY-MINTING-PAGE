...
const AuctionCard = ({ editionAddress, tokenId = 3 }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const imageURI =
        'https://ipfs.io/ipfs/QmNf1UsmdGaMbpatQ6toXSkzDpizaGmC9zfunCyoz1enD5/penguin/3104.png'
      const imageIPFSGateway = ipfsImage(imageURI)
      setEditionsImageSRC(imageIPFSGateway)
      const editionSalesInfo = {
        name: 'Pudgy Penguin #3104',
        symbol: 'PUDGY',
        creator: 'ME',
        maxSupply: '1',
        totalMinted: '1',
        ...
        publicSaleStart: '0',
        publicSaleEnd: '0',
        erc20PaymentToken: '0x67bD6541EB488059C8B84F859b0aD0244F9F0102',
      }
  ...
  const handleBidChange = (event) => {
    ...
    const defaultBid =
      String(chillReservePrice) === String(highestBid)
        ? chillReservePrice
        : BigNumber.from(highestBid).mul(10).div(100).toString()
                    ...
                    <div className="w-full ">
                      <div className=" flex flex-row flex-wrap w-full pb-4 space-y-2 ">
                        <div className="ml-3 mt-3 flex flex-col w-full text-xl">
                          ...
                          {auctionForNFT.highestBidder === address && (
                            <div className="text-sm">(you are the highest bidder)</div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="w-full grid grid-cols-4 ">
                        {canBid && (
                          <Bid
                            initialBid={ethers.utils.formatEther(minimumBid)}
                        ...
                        <div
                          className={`flex flex-row justify-center items-center ${
                            canBid ? 'col-start-3' : 'col-start-1'
                          } col-end-5  text-lg  p-3  w-full h-full border-[1px] border-solid border-[#f70500]`}
                        >
                          ...
                          <>
                            {isActive || !started ? (
                              <CreateBidButton
                                ...
                                nftAddress={editionAddress}
                                bid={bid || minimumBid}
                                onSuccess={updateAuctionData}
                              />
                            ) : (
                              <AuctionSettleButton
                                setPendingTx={setPendingTx}
